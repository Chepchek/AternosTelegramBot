...
def saved_sessions():
    ...
    if len(sessions) > 1:
        logging.warning(
            f"You have more than one saved session, will be used {sessions[0]}"
        )
...
def get_aternos_auth() -> Client:
    try:
        ...
        logging.info(
            "Successfully restored session from aternos_session directory"
        )
        return session
    except (FileNotFoundError, TypeError) as e:
        if config.ATERNOS_PASS:
            logging.info(
                "Successfully authenticated with Aternos login | password"
            )
            session = Client.from_credentials(
                username=config.ATERNOS_LOGIN,
            ...
            logging.info(
                "Successfully authenticated with Aternos login | hash"
            )
            session = Client.from_hashed(
                username=config.ATERNOS_LOGIN,
